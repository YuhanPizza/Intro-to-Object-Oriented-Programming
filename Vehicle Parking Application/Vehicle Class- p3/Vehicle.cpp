...
namespace sdds {
	...
	Vehicle::Vehicle(Vehicle& org) {
		...
		m_model = new char[strlen(org.getMakeModel()) + 1];
		strcpy(m_model, org.getMakeModel());
		m_park = org.getParkingSpot();
	};
	...
	Vehicle& Vehicle::operator=(Vehicle& src) {
		setEmpty();
		strcpy(m_license, src.m_license);
		m_model = new char[strlen(src.m_model) + 1];
		m_park = src.m_park;
	...
	bool operator==(const Vehicle& left, const Vehicle& right) {
		bool value = (strcmp(left.getLicensePlate(), right.getLicensePlate()) == 0);
		return value;
	}
	...
	bool operator==(const Vehicle& left, const char* plate) {
		...
		value = (strcmp(left.getLicensePlate(), temp) == 0);
		return value;
	}
};
